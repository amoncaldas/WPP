...
function cfct_template_file($dir, $file, $data = null) {
	...
	if (file_exists($path)) {
		/*
		if(isset($_SESSION['incnumber']))
		{
			$_SESSION['incnumber']= $_SESSION['incnumber']+1;
		}
		else
		{
		$_SESSION['incnumber']=1;
		
		}
		echo 'inc:'.$_SESSION['incnumber'].'- '.$path.'<br/>';
		*/
		include($path);
	}
	else {
...
function cfct_choose_general_template_single($dir, $files) {
	//echo "context:".cfct_context(); --Amon
