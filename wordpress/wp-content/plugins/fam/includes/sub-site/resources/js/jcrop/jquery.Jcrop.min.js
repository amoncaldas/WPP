...
$.Jcrop = function (obj, opt) {
    ...
    var $trk = $('<div />').addClass(cssClass('tracker')).width(boundx + (bound * 2)).height(boundy + (bound * 2)).css({
        position: 'absolute',
        top: px(-bound),
        left: px(-bound),
        zIndex: 290,
    ...
    var docOffset = getPos(obj),
        btndown, aspectLock, lastcurs, dimmed, animating, shift_down;
    ...
    var Selection = function () {
        ...
        disableHandles();
        $img_holder.append($('<div />').addClass(cssClass('tracker')).mousedown(createDragger('move')).css({
            cursor: 'move',
            position: 'absolute',
            zIndex: 360,
            opacity: 0
    ...
    var KeyManager = function () {
        var $keymgr = $('<input type="radio" />').css({
            position: 'absolute',
            left: '-30px'
        }).keydown(parseKey).keyup(watchShift).blur(onBlur),
            $keywrap = $('<div />').css({
                position: 'absolute',
                overflow: 'hidden'
            }).append($keymgr);
