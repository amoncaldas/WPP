...
require_once(FAM_PLUGIN_PATH."/FAMCore/BO/Atualizacao.php");
...
?>
<section class="atualizacao <?echo $options["add_css_class"];?>"  style="<? echo $hide;?> float:<?echo $options["float"].' !important';?>;margin-left:<?echo $options["margin_left"].' !important'; ?>; margin-right:<?echo $options["margin_right"].' !important'; ?>;width:<?echo $options["width"];?>!important;" >
	<?
		if(is_content_archive('status')) echo '<h1 class="single_type_label status_icon topic_label_icon topic_font_color hand_font">'; else echo "<h2>";
	...
	<ul>
		...
		{			
			...
			{							
			?>
				<li class="item_content <? if($options["show_large_image"] == "yes") echo " largeImage"; ?>">
					<a name="<? echo $atualizacao->AtualizacaoHash;?>"></a>
					<div class="autorInfo">
						<img alt="<? echo $atualizacao->Autor->FullName; ?>" class="foto_viajante_relato" <? if($options["foto_width"] != null) echo "width='".$options["foto_width"]."'"; ?> src="<? echo $atualizacao->Autor->UserImage->ImageGaleryThumbSrc ?>"/>						
					</div>	
									...
									{ 																									
										...
										{			
											...
											else
											{
												if(is_fam_mobile())
												{											
													$mediaSrc = "<img style='width:120px;height:70px;'  title='".$media->Titulo."' src='".$media->YoutubeBaseThumbUrl."1.jpg' alt='".$media->Titulo."' />";												
												}	
												else
												{
													$mediaSrc = "<img style='width:190px;height:140px;'  title='".$media->Titulo."' src='".$media->YoutubeBaseThumbUrl."hqdefault.jpg' alt='".$media->Titulo."' />";
												}
											}																			
										...
										{	
											...
											{	
												if(is_fam_mobile())
												{									
													$mediaSrc = "<img src='".$media->ImageThumbSrc."' alt='".$media->Titulo."' />";
												}
												else
												{
					...
					{															
						widget::Get("share", array("customUrl" => $atualizacao->AtualizacaoUrl , "comment_box_width" => (is_fam_mobile()? "300px": "600px"),'send'=>'true','show_share_bar' => 'yes'));							 
					}
					?>
		...
		{			
			...
			{
				$messagem = $options["excluded_ids"] == null? "Sem atualização de status até o momento" : "Sem outras publicações de status até o momento";
				?><span class="no_content"><?echo $messagem;?><span><?
			}
		}
