...
/**
  ...
  function validate_captcha($recaptchaToken) {
		...
		$wpp_recaptcha_site_key = get_option("wpp_recaptcha_site_key");

		// If no recaptcha_site_key is defined, then it is defined that 
		// we must not use recaptcha to validate the comment insert
		if (!$wpp_recaptcha_site_key) {
			return true;
		}

		$secret = get_recaptcha_secret();
		
		// If there is not recaptcha secret defined
		// then we can validate. we will consider
		// that recaptcha is disabled
		// and return true
		if (!$secret) {
			return false;
		}
