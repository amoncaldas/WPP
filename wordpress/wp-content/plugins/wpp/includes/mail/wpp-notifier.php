  ...
  public function register_routes() {
	...
	public function report_error($request) {
		...
		if ($_SERVER['HTTP_REFERER'] === $current_url) {
			...
			if (isset($subject) && isset($message)) {
				$title = $subject;
				WppMailer::notify_admin($title, $message, get_default_locale());
	...
	public function subscribe_for_notifications($request) {
		...
		if (isset($name) && isset($email)) {
			$lang = $request->get_param(self::$lang_tax_slug) ? $request->get_param(self::$lang_tax_slug) : get_default_locale();
