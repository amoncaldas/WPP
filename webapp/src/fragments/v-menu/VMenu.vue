<template>
    ...
    <v-expansion-panel-content class="v-menu-expansion-panel">
      ...
      <template v-else-if="showMenuItem(item)">
        <template v-for="itemLevel2 in item.items">
          ...
          <v-list-tile v-else-if="itemLevel2" :key="itemLevel2.href" @click.prevent="nav(itemLevel2)" :href="itemLevel2.href" class="v-menu-item" :class="itemClass" :title="itemLevel2.title">
            <v-list-tile-action>
  ...
  <v-list-tile :key="item.href" v-else-if="showMenuItem(item)" class="v-menu-item" :class="itemClass" :href="item.href" @click.prevent="nav(item)" ripple v-bind:disabled="item.disabled"
    :title="item.title">
    <v-list-tile-action>
      <v-icon class="notranslate"> {{ item.icon }} </v-icon>
    </v-list-tile-action>
